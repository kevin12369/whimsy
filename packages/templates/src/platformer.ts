...
function buildHtml(theme: Theme, levelDataJson: string, bossDataJson: string, howToPlay: string, scoreKey: string): string {
...
<script>
(function(){
  const COLOR=${c}, WHITE=${w}, PLAYER='${theme.playerLabel}', ENEMY='${theme.enemyLabel}';
  const SCORE_KEY='${scoreKey}';
  const LEVELS=${levelDataJson};
  const BOSS=${bossDataJson};
  let score=0, lives=3, gameOver=false, currentLevel=0, levelReady=true, bossActive=false;
  let player, cursors, ground, platforms, enemies, stars, goal, scoreText, livesText, levelText, boss;

  function loadHigh(){
    try{return JSON.parse(localStorage.getItem(SCORE_KEY)||'{"high":0}').high}catch(e){return 0}
  }
  function saveHigh(s){
    try{localStorage.setItem(SCORE_KEY,JSON.stringify({high:Math.max(s,loadHigh())}))}catch(e){}
  }
  function updateHud(){
    const h=document.getElementById('hud');
    h.innerHTML='HOW TO PLAY: ${howToPlay.replace(/'/g, "\\'")} | Level: '+(currentLevel+1)+'/3 | Score: '+score+' | High: '+loadHigh();
  }

  function newGame(){
    new Phaser.Game({type:Phaser.AUTO,parent:'g',width:800,height:480,
      physics:{default:'arcade',arcade:{gravity:{y:LEVELS[currentLevel].gravity}}},
      scene:{
        create(){
          ...
          for(const e of lv.enemies){const r=this.add.rectangle(e.x,e.y-15,24,24,0xff4444);this.physics.add.existing(r);enemies.add(r);r.body.setVelocity(e.sp,0)}
          ...
          this.physics.add.overlap(player,boss,()=>{if(!boss)return;if(player.body.touching.down&&player.body.velocity.y>200){boss.hp--;player.body.setVelocityY(-400);if(boss.hp<=0){boss.destroy();boss=null;score+=200;updateHud();setTimeout(()=>nextLevel(this),500)}else{score+=50;updateHud()}}else hit()});
          updateHud();
        },
        update(){
...
export const sideScrollerComet: Template = {
  ...
  render(theme: Theme): string {
    return buildHtml(
      theme,
      JSON.stringify(LEVEL_DATA.sideScrollerComet),
      JSON.stringify(BOSS_DATA.sideScrollerComet),
      sideScrollerComet.howToPlay,
      'whimsy:score:sideScrollerComet',
    );
  },
};
