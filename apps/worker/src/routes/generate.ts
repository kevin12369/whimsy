...
import { pickProvider, LlmError } from '@whimsy/llm';
...
generate.post('/api/generate', async (c) => {
  ...
  try {
    provider = isLocal
      ? pickProvider(model, c.env, {
          baseUrl: body.localBaseUrl,
          model: body.localModel ?? 'default',
          apiKey: body.localApiKey,
          timeoutMs: body.localTimeoutMs,
        })
      : pickProvider(model, c.env);
  } catch (e) {
    if (e instanceof LlmError || isLocal) {
